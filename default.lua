...
-- a subset of osm objects that are considered to be points of interest
...
local function has_area_tags(tags)
    ...
    return tags.aeroway or tags.amenity or tags.building or tags.harbour or tags.historic or tags.landuse or
               tags.leisure or tags.man_made or tags.military or tags.natural or tags.office or tags.place or tags.power or
               tags.public_transport or tags.shop or tags.sport or tags.tourism or tags.water or tags.waterway or
               tags.wetland or tags['abandoned:aeroway'] or tags['abandoned:amenity'] or tags['abandoned:building'] or
               tags['abandoned:landuse'] or tags['abandoned:power'] or tags['area:highway'] or tags['building:part'] or
               tags.landcover or tags.highway == 'rest_area' or tags.highway == 'services'
end
...
-- tag-value pairs that should always be considered POIs
local included_poi_tags = {
    railway = {
        station = true
    },
    aeroway = {
...
local function is_poi(tags)
    -- If any excluded key-value pair exists, it's never a POI
    for excluded_key, values in pairs(excluded_poi_tags) do
        if tags[excluded_key] and values[tags[excluded_key]] then
            return false
        end
    end

    -- Check always-included tag-value pairs
    for key, values in pairs(included_poi_tags) do
        local tag_value = tags[key]
        if tag_value and values[tag_value] then
            return true
        end
    end

    -- Check if any standard POI key is present
...
function osm2pgsql.process_node(object)
    ...
    if is_poi(object.tags) == true then
        tables.pois:insert({
            tags = object.tags,
...
function osm2pgsql.process_way(object)
    ...
    if object.is_closed and has_area_tags(object.tags) then
        ...
        if is_poi(object.tags) == true then
            tables.pois:insert({
                tags = object.tags,
                point = object:as_polygon():centroid(),
                area = object:as_polygon(),
                version = object.version
            })
        end
    else
        ...
        if is_poi(object.tags) == true then
            tables.pois:insert({
                tags = object.tags,
                point = object:as_linestring():centroid(),
                version = object.version
            })
        end
    end
...
function osm2pgsql.process_relation(object)
    ...
    if relation_type == 'multipolygon' then
        ...
        if is_poi(object.tags) == true then
            tables.pois:insert({
                tags = object.tags,
                point = object:as_multipolygon():centroid(),
                area = object:as_multipolygon(),
                version = object.version
            })
        end
    end
