...
export default async function StreetIndexPage({
  ...
  const response = await pool.query(
    `SELECT p.*, f.name as feature, f.color as color FROM pois p left join feature f on f.id = p."featureId" WHERE p.country = $1 AND p.state = $2 AND p.city = $3 AND p.street = $4 ORDER BY p.name ASC, f.importance DESC`,
